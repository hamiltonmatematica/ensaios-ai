...
import { getGoogleAdsCreds, listGoogleAdsAdGroupRows } from '@/lib/google-ads';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ campaignId: string }> }) {
    try {
        ...
        const { rows, errors } = await listGoogleAdsAdGroupRows(creds, campaignId, range, prev);

        return NextResponse.json({ success: true, data: rows, partialErrors: errors.length ? errors : undefined });
    } catch (error: any) {
        return NextResponse.json(
            { success: false, error: error.message || 'Erro ao buscar grupos de anúncios do Google Ads' },
            { status: 500 }
        );
    }
}
