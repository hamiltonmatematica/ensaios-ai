...
import { getGoogleAdsCreds, listGoogleAdsCampaignRows } from '@/lib/google-ads';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ accountId: string }> }) {
    try {
        ...
        const { rows, errors } = await listGoogleAdsCampaignRows(creds, accountId, range, prev);

        return NextResponse.json({ success: true, data: rows, partialErrors: errors.length ? errors : undefined });
    } catch (error: any) {
        return NextResponse.json(
            { success: false, error: error.message || 'Erro ao buscar campanhas do Google Ads' },
            { status: 500 }
        );
    }
}
